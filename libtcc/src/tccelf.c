...
ST_FUNC void tccelf_end_file(TCCState* s1) {
  ...
  for (i = 0; i < nb_syms; ++i) {
    ...
    tr[i] = set_elf_sym(
        s, sym->st_value, sym->st_size, sym->st_info, sym->st_other, sym->st_shndx, s->link->data + sym->st_name);
  }
...
ST_FUNC int
put_elf_sym(Section* s, addr_t value, unsigned long size, int info, int other, int shndx, const char* name) {
...
ST_FUNC int
set_elf_sym(Section* s, addr_t value, unsigned long size, int info, int other, int shndx, const char* name) {
  ElfW(Sym) * esym;
...
ST_FUNC void squeeze_multi_relocs(Section* s, size_t oldrelocoffset) {
  ...
  for (a = oldrelocoffset + sizeof(*r); a < sr->data_offset; a += sizeof(*r)) {
    ...
    for (; i >= (ssize_t)oldrelocoffset && ((ElfW_Rel*)(sr->data + i))->r_offset > addr; i -= sizeof(*r)) {
...
static void build_got(TCCState* s1) {
  ...
  s1->got->sh_entsize = 4;
  set_elf_sym(symtab_section, 0, 4, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0, s1->got->sh_num, "_GLOBAL_OFFSET_TABLE_");
...
static struct sym_attr* put_got_entry(TCCState* s1, int dyn_reloc_type, unsigned long size, int info, int sym_index) {
  int need_plt_entry;
  const char* name;
  ElfW(Sym) * sym;
  ...
  if (s1->dynsym) {
    if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
      ...
      if (0 == attr->dyn_index)
        attr->dyn_index = set_elf_sym(s1->dynsym, sym->st_value, size, info, 0, sym->st_shndx, name);
  ...
  if (need_plt_entry) {
    ...
    attr->plt_sym = put_elf_sym(
        s1->symtab, attr->plt_offset, sym->st_size, ELFW(ST_INFO)(STB_GLOBAL, STT_FUNC), 0, s1->plt->sh_num, plt_name);
...
static void add_init_array_defines(TCCState* s1, const char* section_name) {
  ...
  set_elf_sym(symtab_section, end_offset, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, s->sh_num, sym_end);
}
#endif
...
static void tcc_add_linker_symbols(TCCState* s1) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if (s->sh_type == SHT_PROGBITS && (s->sh_flags & SHF_ALLOC)) {
      ...
      set_elf_sym(symtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, s->sh_num, buf);
      snprintf(buf, sizeof(buf), "__stop_%s", s->name);
      set_elf_sym(symtab_section, s->data_offset, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, s->sh_num, buf);
    }
  next_sec:;
  }
}
...
static int alloc_sec_names(TCCState* s1, int file_type, Section* strsec) {
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if (file_type == TCC_OUTPUT_DLL && s->sh_type == SHT_RELX && !(s->sh_flags & SHF_ALLOC) &&
        (s1->sections[s->sh_info]->sh_flags & SHF_ALLOC) && prepare_dynamic_rel(s1, s)) {
      if (s1->sections[s->sh_info]->sh_flags & SHF_EXECINSTR)
        textrel = 1;
    } else if (s1->do_debug || file_type == TCC_OUTPUT_OBJ || (s->sh_flags & SHF_ALLOC) || i == (s1->nb_sections - 1)) {
...
static void tcc_output_elf(TCCState* s1, FILE* f, int phnum, ElfW(Phdr) * phdr, int file_offset, int* sec_order) {
...
static int
tcc_write_elf_file(TCCState* s1, const char* filename, int phnum, ElfW(Phdr) * phdr, int file_offset, int* sec_order) {
...
#endif
        sh->sh_type != SHT_NOBITS && sh->sh_type != SHT_PREINIT_ARRAY && sh->sh_type != SHT_INIT_ARRAY &&
        sh->sh_type != SHT_FINI_ARRAY && strcmp(sh_name, ".stabstr"))
      continue;
    if (seencompressed && (!strncmp(sh_name, ".debug_", sizeof(".debug_") - 1) ||
                           (sh->sh_type == SHT_RELX &&
                            !strncmp((char*)strsec + shdr[sh->sh_info].sh_name, ".debug_", sizeof(".debug_") - 1))))
      continue;
    ...
    switch (s->sh_type) {
      case SHT_RELX:
        ...
        for_each_elem(s, (offset / sizeof(*rel)), rel, ElfW_Rel) {
          ...
          invalid_reloc:
            tcc_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x", i, strsec + sh->sh_name, rel->r_offset);
            goto fail;
          }
          rel->r_info = ELFW(R_INFO)(sym_index, type);
  ...
  do {
    ...
    for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p) + 1) {
      ...
      if (sym_index) {
        sym = &((ElfW(Sym)*)symtab_section->data)[sym_index];
        if (sym->st_shndx == SHN_UNDEF) {
          off = (entrysize == 4 ? get_be32(ar_index + i * 4) : get_be64(ar_index + i * 8)) + sizeof(ArchiveHeader);
          ++bound;
