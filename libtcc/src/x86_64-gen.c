...
static void gcall_or_jmp(int is_jmp) {
  ...
  if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST &&
    ...
    if (vtop->r & VT_SYM) {
      ...
      put_elf_reloca(symtab_section, cur_text_section, ind + 1, R_X86_64_PC32, 0, (int)(vtop->c.i - 4));
    }
...
#define REGN 6
static const uint8_t arg_regs[REGN] = {TREG_RDI, TREG_RSI, TREG_RDX, TREG_RCX, TREG_R8, TREG_R9};
...
void gen_cvt_itof(int t) {
  if ((t & VT_BTYPE) == VT_LDOUBLE) {
    ...
    if ((vtop->type.t & (VT_BTYPE | VT_UNSIGNED)) == (VT_INT | VT_UNSIGNED) || (vtop->type.t & VT_BTYPE) == VT_LLONG) {
      o(0x48); /* REX */
    }
    o(0x2a0f);
