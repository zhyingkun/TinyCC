...
static void* win64_add_function_table(TCCState* s1) {
  ...
  if (s1->uw_pdata) {
    ...
    RtlAddFunctionTable((RUNTIME_FUNCTION*)p, s1->uw_pdata->data_offset / sizeof(RUNTIME_FUNCTION), s1->pe_imagebase);
    s1->uw_pdata = NULL;
  }
  return p;
}
    ...
    for (sym = (ElfW(Sym)*)symtab_section->data + 1; sym < sym_end; sym++) {
      ...
      if (type == STT_FUNC || type == STT_GNU_IFUNC) {
        if (wanted_pc >= sym->st_value && wanted_pc < sym->st_value + sym->st_size) {
          pstrcpy(last_func_name, sizeof(last_func_name), (char*)symtab_section->link->data + sym->st_name);
