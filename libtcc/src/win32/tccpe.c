...
enum { sec_text = 0, sec_data, sec_bss, sec_idata, sec_pdata, sec_other, sec_rsrc, sec_stab, sec_reloc, sec_last };
...
#define CHARACTERISTICS_DLL 0x230F
  ...
  pe->sizeofheaders = pe_file_align(pe, sizeof(struct pe_header) + pe->sec_count * sizeof(IMAGE_SECTION_HEADER));
  ...
  for (i = 0; i < pe->sec_count; ++i) {
    ...
    if (pe->thunk == pe->s1->sections[si->ord]) {
      if (pe->imp_size) {
        ...
        pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_IAT, pe->iat_offs + addr, pe->iat_size);
      }
      if (pe->exp_size) {
        pe_set_datadir(&pe_header, IMAGE_DIRECTORY_ENTRY_EXPORT, pe->exp_offs + addr, pe->exp_size);
      }
    }
    ...
    pe_header.opthdr.SizeOfImage = umax(pe_virtual_align(pe, size + addr), pe_header.opthdr.SizeOfImage);
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (offset) {
          ...
          *(DWORD*)(p + 1) = (DWORD)-4;
          ...
          is->iat_index = put_elf_sym(
              symtab_section, 0, sizeof(DWORD), ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0, SHN_UNDEF, buffer);
#ifdef TCC_TARGET_ARM
          put_elf_reloc(
              symtab_section, text_section, offset + 8, R_XXX_THUNKFIX, is->iat_index); // offset to IAT position
#else
    ...
    not_found:
      if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
        ...
        continue;
      tcc_error_noabort("undefined symbol '%s'%s", name, imp_sym < 0 ? ", missing __declspec(dllimport)?" : "");
      ret = -1;
...
ST_FUNC int pe_putimport(TCCState* s1, int dllindex, const char* name, addr_t value) {
  return set_elf_sym(s1->dynsymtab_section,
                     value,
                     dllindex, /* st_size */
...
static unsigned pe_add_uwwind_info(TCCState* s1) {
  ...
  if (0 == s1->uw_offs) {
    ...
    static const unsigned char uw_info[] = {
        ...
        0x04,
        0x03, // set frame ptr (mov rsp -> rbp)
        0x01,
        0x50 // push reg (rbp)
    };
...
#endif
    set_elf_sym(symtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_UNDEF, start_symbol);

  tcc_add_pragma_libs(s1);

  if (0 == s1->nostdlib) {
    static const char* libs[] = {TCC_LIBTCC1, "msvcrt", "kernel32", "", "user32", "gdi32", NULL};
    const char **pp, *p;
