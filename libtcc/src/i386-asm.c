...
static void asm_rex(int width64, Operand* ops, int nb_ops, int* op_type, int regi, int rmi) {
  ...
  if (rex) {
    if (saw_high_8bit)
      tcc_error("can't encode register %%%ch when REX prefix is required", "acdb"[saw_high_8bit - 4]);
    g(rex);
  }
}
#endif
...
again:
  ...
  for (pa = asm_instrs; pa->sym != 0; pa++) {
    ...
    if (pa->opcode == 0xb0 && ops[0].type != OP_IM64 && (ops[1].type & OP_REG) == OP_REG64 &&
  ...
  if (s == autosize) {
    ...
    if (s == autosize) {
      if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) && (ops[0].type & (OP_SEG | OP_IM8S | OP_IM32)))
        s = 2;
      else if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) && (ops[0].type & OP_EA))
        s = NBWLX - 2;
      else
  ...
  modrm_found:
    ...
    for (i = 0; i < nb_ops; i++) {
      int t = op_type[i];
      if (i != modrm_index && (t & (OP_REG | OP_MMX | OP_SSE | OP_CR | OP_TR | OP_DB | OP_SEG))) {
        modreg_index = i;
        break;
      }
    }
