...
ST_DATA int vla_sp_loc; /* Pointer to variable holding location to store stack pointer on the stack when modifying stack
                           pointer */
...
static void gen_opic(int op) {
  ...
  if (c1 && c2) {
    ...
    if (!const_wanted && c1 &&
        ((l1 == 0 && (op == TOK_SHL || op == TOK_SHR || op == TOK_SAR)) || (l1 == -1 && op == TOK_SAR))) {
    ...
    case VT_PTR:
      ...
      if ((type1->t & VT_BTYPE) == VT_VOID || (type2->t & VT_BTYPE) == VT_VOID) {
        ...
        if (!is_compatible_unqualified_types(type1, type2)) {
          ...
          if ((type1->t & (VT_BTYPE | VT_LONG)) != (type2->t & (VT_BTYPE | VT_LONG)) || IS_ENUM(type1->t) ||
              IS_ENUM(type2->t))
            tcc_warning("assignment from incompatible pointer type");
        }
      }
    ...
    if (f->v & SYM_FIRST_ANOM && (f->type.t & VT_BTYPE) == VT_STRUCT) {
      ...
      if (!(v2 & SYM_FIELD) && (v2 & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
        ...
        /* This happens only with MS extensions.  The
           ...
           them.  */
        ass = f->type.ref;
        f->type.ref = sym_push(anon_sym++ | SYM_FIELD, &f->type.ref->type, 0, f->type.ref->c);
        pps = &f->type.ref->next;
...
static void expr_eq(void) {
  ...
  expr_cond();
  if (tok == '=' || (tok >= TOK_A_MOD && tok <= TOK_A_DIV) || tok == TOK_A_XOR || tok == TOK_A_OR || tok == TOK_A_SHL ||
      tok == TOK_A_SAR) {
    test_lvalue();
    t = tok;
    next();
...
static void gfunc_return(CType* func_type) {
  if ((func_type->t & VT_BTYPE) == VT_STRUCT) {
    ...
    if (0 == ret_nregs) {
      ...
      if ((vtop->r != (VT_LOCAL | VT_LVAL) || (vtop->c.i & (ret_align - 1))) && (align & (ret_align - 1))) {
